...
function $(v,o) 
// Returns the element with the ID of 'v', either within the object 'o' or the 
// document, if 'o' is not an object
{ 
...
function $S(o) 
// Returns the style of the object or element 'o', where the element is 
    ...
    { 
        ...
        { 
            ...
            else 
            {
                ...
                // Set starCur to the width of the mouse position
                $S('starCur' + n).width = Math.round(oX) + 'px'; 

                // Set the digit color to dark gray
                $S('text' + n).color = '#111'; 

                // Set the text of the percent digit
                $('text' + n).innerHTML = Math.round(oX / 84 * 100 / 10)*10 + '%'; 
            }
        };
    } 
};
...
{
    // 'v' is the digit text
    var n = star.num, v = parseInt($('text' + n).innerHTML);
    n = o.id.substr(4);
